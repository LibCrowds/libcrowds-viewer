...
Generate Transcribe Annotations by setting the task options `type` to
`transcribe`; see the [Configuration](../configuration.md) section for details.

Note that the `form` task property is required when in `transcribe` mode.
The `tag` and `classification` task properties are ignored and are instead
